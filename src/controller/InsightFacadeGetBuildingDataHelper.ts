...
export default class InsightFacadeGetBuildingDataHelper {
    ...
    public getRoomTopData(): any {
        return this.roomTopData;
    }
    ...
    public extractRoomsData(tbody: any): any[] {
        let roomsData: any[] = [];
        for (let e of tbody.childNodes) {
            if (e.nodeName === "tr") {
        ...
        for (let e of tr.childNodes) {
            if (e.nodeName === "td") {
                if (this.hasAttrField(e.attrs, "class",
                    ...
                    roomData.fullname = this.extractInnerData(e);
                }
            }
        }
        return roomData;
    ...
    public getTableFromBody(body: any) {
    ...
    private getTableFromSection(section: any) {
        for (let e of section.childNodes) {
            ...
            if (e.nodeName === "div" && this.hasAttrField(e.attrs, "class",
                ...
                return this.getTableFromDivs(e); // For rooms
            }
        }
    }

    private getTableFromDivs(e: any) {
        for (let e1 of e.childNodes) {
            ...
            } else if (e1.nodeName === "div" && this.hasAttrField(e1.attrs, "class", "view-content")) {
                for (let e2 of e1.childNodes) {
                    if (e2.nodeName === "div" && this.hasAttrField(e2.attrs, "class",
                        "views-row views-row-1 views-row-odd views-row-first views-row-last")) {
                        this.getTableFromInnerDivs(e2);
                    }
                }
            }
        }
            ...
            if (e3.nodeName === "div" && this.hasAttrField(e3.attrs, "class", "view-content")) {
                ...
                for (let e4 of e3.childNodes) {
                    if (e4.nodeName === "div" && this.hasAttrField(e4.attrs, "id", "building-info")) {
                        return this.storeRoomAddress(e4);
                    }
                }
            }
        }
    ...
    private storeRoomAddress(div: any) {
        let temp: any[] = [];
        for (let e of div.childNodes) {
